...
db.mongoose.connection.once('open', () => {
  ...
  db.models.JackwuiConfigs.findOne({}, (err, configs) => {

    let setupsshkeys = configs => {
      if(!configs.verifySSHKey())
      configs.createSSHKey();
    };

    if (!configs) {
      configs = db.models.JackwuiConfigs({
        name: "artwo Jackwui"
      });
      configs.save( err => {
        setupsshkeys(configs);
      });
    } else {
      setupsshkeys(configs);
    }
...
//  let oneServer = new db.models.JackNodes({
//     name: "artwo", uri: "path.tech", user: "jack"
//  });
//   oneServer.save();
